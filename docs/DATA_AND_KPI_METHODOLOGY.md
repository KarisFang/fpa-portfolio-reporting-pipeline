...
is joined against those ranges to land in the right bucket — that's the core SQL technique used to turn
uniform randomness into realistic, biased synthetic data, entirely in SQL.
...
computing it once. The fix is the `MATERIALIZED` keyword (SQLite 3.35+), which forces the CTE to be computed
